<?php $title = "Permohonan Gabungan"; $location_index = "../.."; include('../../components/head.php')?>
...
<script>
    ...
    rows: [
        <?php 
            ...
            $pembelian_baru_sql = $pdo->prepare("SELECT * FROM pembelian WHERE status_pembelian = 1");
            $pembelian_baru_sql->execute([]);
            
            $no = 0;
            while($pembelian_baru = $pembelian_baru_sql->fetch(PDO::FETCH_ASSOC)){
                $no++;

                $id_pembelian = $pembelian_baru['id_pembelian'];
                $tarikh_cipta_pembelian = $pembelian_baru['tarikh_cipta_pembelian'];
                $harga_anggaran_pembelian = $pembelian_baru['harga_anggaran_pembelian'];

                $bil_permohonan_pembelian = 0;

                $permohonan_pembelian = rtrim($pembelian_baru['id_permohonan_pembelian'], ', ');
