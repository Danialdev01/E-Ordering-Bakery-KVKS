...
<script>
    ...
    rows: [
        <?php 
            ...
            $pembelian_selesai_sql = $pdo->prepare("SELECT * FROM pembelian WHERE status_pembelian = 2");
            $pembelian_selesai_sql->execute([]);
            
            $no = 0;
            while($pembelian_selesai = $pembelian_selesai_sql->fetch(PDO::FETCH_ASSOC)){
                $no++;

                $id_pembelian = $pembelian_selesai['id_pembelian'];
                $tarikh_cipta_pembelian = $pembelian_selesai['tarikh_cipta_pembelian'];
                $harga_anggaran_pembelian = $pembelian_selesai['harga_anggaran_pembelian'];

                $bil_permohonan_pembelian = 0;

                $permohonan_pembelian = rtrim($pembelian_selesai['id_permohonan_pembelian'], ', ');
